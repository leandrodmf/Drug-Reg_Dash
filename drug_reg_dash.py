...
def create_ther_class_plot(df_allregs_tc):
    ...
    df_melted = df_allregs_tc.reset_index().melt(
        id_vars='YEAR',
        var_name='THERAPEUTIC_CLASS',
        value_name='Registers'
    )
    ...
    # Ajustar layout
    fig.update_layout(
        width=1000, height=500, font_family='DejaVu Sans', font_size=15,
        font_color=GRAY2, title_font_color=GRAY1, title_font_size=24,
        title_text='Brazil Drug Registrations: by Therapeutical Class' +
                   '<br><sup size=1 style="color:#555655">From 1995 to 2024</sup>',
        xaxis_title='', yaxis_title='', plot_bgcolor=GRAY5
    )
    fig.update_traces(
        mode="markers+lines",
        hovertemplate="<b>Year:</b> %{x} <br> <b>Registers:</b> %{y}"
    )
    fig.update_layout(hovermode="x unified")
...
def prepare_therapeutic_class_data(df):
    top_5_tc = df['THERAPEUTIC_CLASS'].value_counts().nlargest(5).index
    df_allregs_tc = df[['PROCESS_FINALIZATION_DATE', 'PRODUCT_REGISTRATION_NUMBER', 'THERAPEUTIC_CLASS']].copy()
    df_allregs_tc = df_allregs_tc[df_allregs_tc['THERAPEUTIC_CLASS'].isin(top_5_tc)]
    df_allregs_tc['YEAR'] = pd.to_datetime(df_allregs_tc['PROCESS_FINALIZATION_DATE'], errors='coerce').dt.year

    # Verifique se existem dados após o filtro
    if df_allregs_tc.empty:
        raise ValueError("No data available after filtering by therapeutic class.")
    ...
    # Renomeie colunas
    df_allregs_tc = df_allregs_tc.rename(columns={
        'ANALGESICOS NAO NARCOTICOS': 'Non-opioid analgesic',
        'ANTIBIOTICOS SISTEMICOS SIMPLES': 'Systemic antibiotics simple',
        'ANTIDEPRESSIVOS': 'Antidepressant',
        'ANTINEOPLASICO': 'Antineoplastic',
        'ANTINFLAMATORIOS': 'Non-steroidal anti-inflammatory'
    ...
    if df_allregs_tc.empty:
        raise ValueError("No data available after filtering by year >= 1995.")
...
def create_violin_plot(df_allregs_tc):
     ...
     # Verificar se a mediana contém valores suficientes
     if len(median) < 2:
        raise ValueError("Not enough data to calculate medians for the violin plot.")
     # Ajuste no texto
     median_text = f"with median fluctuating from {median.iloc[-2]:.1f} to {median.iloc[-1]:.1f}."
...
def main():
 ...
 st.markdown("""
             ...
             Data download: [Medicamentos Registrados no Brasil](https://dados.gov.br/dados/conjuntos-dados/medicamentos-registrados-no-brasil)
